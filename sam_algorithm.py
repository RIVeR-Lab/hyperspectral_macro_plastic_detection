...
def mean_pixel_values_for_label_id(mask, label_id_mask, rgb, hsi):
    ...
    """
    rgb_means = [np.mean(np.multiply(rgb[:, :, channel], label_id_mask)) /
                 np.mean(label_id_mask) for channel in range(rgb.shape[2])]
    hsi_means = [np.mean(np.multiply(np.nan_to_num(hsi[:, :, channel]), label_id_mask)
                         )/np.mean(label_id_mask) for channel in range(hsi.shape[2])]

    return rgb_means, hsi_means


def all_pixel_values_for_label_id(mask, label_id_mask, rgb, hsi):
    """
    Calculate the mean pixel values for a specific label ID.
    ...
    hsi_means = [np.nan_to_num(hsi[:, :, channel]).flatten(
    )[label_id_mask.flatten()] for channel in range(hsi.shape[2])]
...
rescale = 4
accs = []
for img in training_mask_paths:
    unique_labels = np.unique(np.concatenate(([np.unique(cv2.imread(img))])))
    print(img, unique_labels)
...
for label in unique_labels:
    ref_vect[label] = []
    ref_vect_all_pixels[label] = np.zeros((33, 0))
    ref_vect_kmeans[label] = []
...
for plot_index, mask_path in enumerate(training_mask_paths):
    ...
    for index, label_id in enumerate(np.unique(mask)):
        ...
        rgb_means, hsi_means = mean_pixel_values_for_label_id(
            mask, label_specific_mask, rgb, hsi)
        ref_vect[label_id].append(hsi_means)
        rgb_pixels, hsi_pixels = all_pixel_values_for_label_id(
            mask, label_specific_mask, rgb, hsi)
        ref_vect_all_pixels[label_id] = np.concatenate((ref_vect_all_pixels[label_id],
                                                        np.array(hsi_pixels)), axis=1)

n_clusters_per_label = 10

for label in unique_labels:
    # print(ref_vect_all_pixels[label].shape)
    kmeans = KMeans(n_clusters=n_clusters_per_label, random_state=0,
                    n_init="auto").fit(ref_vect_all_pixels[label].T)
    # print(kmeans.cluster_centers_)
    print(kmeans.cluster_centers_.shape)
    ref_vect_kmeans[label] = kmeans.cluster_centers_
    # break
...
#     for index, label_id in enumerate(np.unique(mask)):
#         label_specific_mask = mask_for_each_label(mask, label_id)
#         _, hsi_means = mean_pixel_values_for_label_id(mask, label_specific_mask, rgb, hsi)
#         ref_vect[label_id].append(hsi_means)
#     print(ref_vect)
...
def detect(mask_paths, ref_vect):
    ...
    # run classification
    fig, axes = plt.subplots(len(mask_paths)*2, 5, figsize=(4, 5))
    ...
    for plot_index, mask_path in enumerate(mask_paths):
        ...
        mask = mask[0:x, 0:y]
        is_plastic_mask = np.ceil(mask/254) % 2
        ref_count = len(ref_vect)*n_clusters_per_label
        ...
        for x in range(ix):
            for y in range(iy):
                p_ang = [0] * ref_count
                j = 0
                for asd, j_label in enumerate(ref_vect):
                    for cluster in range(n_clusters_per_label):
                        p_mag = [0] * ref_count
                ...
                # if min(p_ang) < 0.2:
                print(len(p_ang))
                class_no = np.argmin(p_ang, axis=0) // n_clusters_per_label
                # else:
        ...
        acc = np.sum(is_plastic_mask == is_plastic_prediction) / \
            (is_plastic_prediction.shape[0]*is_plastic_prediction.shape[1])
        accs.append(acc)
...
detect(training_mask_paths, ref_vect_kmeans)
detect(test_mask_paths, ref_vect_kmeans)
