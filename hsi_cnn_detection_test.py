...
class hsi_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        hsi_path, _ = self.t.find_pairs(self.paths_[index])
        mask_img = self.t.load_mask(self.paths_[index], 4)
        hsi_img = self.t.load_hsi(hsi_path, 4)
        x = min(mask_img.shape[0], hsi_img.shape[0])
        y = min(mask_img.shape[1], hsi_img.shape[1])
        imgA = hsi_img[0:x, 0:y]
        ...
        imgA = imgA.clone().detach().to(torch.float)
        imgB = imgB.to(torch.float32)
...
def fcn_train():
    ...
    train_dataset = hsi_dataset(training_mask_paths, fcn_transform)
...
def fcn_test(fcn_model, test_path):
    fcn_model.eval()
    test_dataset = hsi_dataset(test_path, fcn_transform)
...
if __name__ == "__main__":
    # fcn_model = fcn_train()
    # torch.save(fcn_model,"../cnn_models/fcn_hsi_model1.pth")

    fcn_model = torch.load("../cnn_models/fcn_hsi_model1.pth")
    fcn_test(fcn_model,[training_mask_paths[5]])
